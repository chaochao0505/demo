 ...
 *
 * @author guojy
 * @version V1.0.0-RELEASE 日期：2016年8月16日
 * @since 1.0.0-RELEASE
 */
public class DateUtil extends DateUtils {
     ...
     * @return
     */
    public static long getDateTime(String strDateTime)
    {
        long lReturn = 0;
        try
        {
            DateFormat df = DateFormat.getDateTimeInstance();
            lReturn = df.parse(strDateTime).getTime();
        }
     ...
     * 在日期上增加分钟
     */
    public static String addMinute(String dateTime, String informate, String outformate,
                                   Integer flag) {
        Date dataDate = null;
        try {
            dataDate = formatStrToDate(dateTime, informate);
     ...
     * 在日期上增加秒
     */
    public static String addSecond(String dateTime, String informate, String outformate,
                                   Integer flag) {
        Date dataDate = null;
        try {
            dataDate = formatStrToDate(dateTime, informate);
    ...
    public static String getFirstDayByTime(String dateTime, String inFormat, String outFormat) {
        // 获取转换后的时间
        String transformTime = DateUtil.transformStrFormat(dateTime, inFormat,
            DateUtil.DAY_UNINTERVAL);
        if (transformTime != null && transformTime.trim().length() == 8) {
            transformTime = transformTime.substring(0, 6) + "01";
        }
        // 把当前月第一天时间转换格式返回
        transformTime = DateUtil.transformStrFormat(transformTime, DateUtil.DAY_UNINTERVAL,
            outFormat);
    ...
    public static String getLastDayByMonth(String yearMonth, String inFormat, String outFormat) {
        ...
        try {
            ...
            if ( (nowTime.get(Calendar.YEAR) == cTime.get(Calendar.YEAR))
                && (nowTime.get(Calendar.MONTH) == cTime.get(Calendar.MONTH))) {
     ...
     */
    public static String getDayOfNextWeek(Integer year, Integer weekOfYear, Integer dayOfWeek,
                                          Integer flag) {
        DateFormat df = new SimpleDateFormat(DateUtil.DAY_HAVINTERVAL);
        Calendar rightNow = Calendar.getInstance();
        rightNow.set(Calendar.YEAR, year);
    ...
    public static int getNextWeekOrderByWeekOrder(Integer weekOrder, Integer flag) throws Exception {
        ...
        if (null == flag) {// 向下计算
            return getWeekOrderByDate(getDayOfNextWeek(year, weekOfyear, Calendar.SATURDAY, null),
                DateUtil.DAY_HAVINTERVAL);
        } else {
            return getWeekOrderByDate(getDayOfNextWeek(year, weekOfyear, Calendar.SATURDAY, 2),
                DateUtil.DAY_HAVINTERVAL);
        }
    }
    ...
    public static String getBeginDateBySeasonId(String seasonId, String format) {
        if ("01".equalsIgnoreCase(seasonId.substring(4, 6))) {// 春
            return DateUtil.getFirstDayByMonth(seasonId.substring(0, 4) + "01",
                DateUtil.MONTH_UNINTERVAL, format);
        } else if ("02".equalsIgnoreCase(seasonId.substring(4, 6))) {// 夏
            return DateUtil.getFirstDayByMonth(seasonId.substring(0, 4) + "04",
                DateUtil.MONTH_UNINTERVAL, format);
        } else if ("03".equalsIgnoreCase(seasonId.substring(4, 6))) {// 秋
            return DateUtil.getFirstDayByMonth(seasonId.substring(0, 4) + "07",
                DateUtil.MONTH_UNINTERVAL, format);
        } else {// 冬
            return DateUtil.getFirstDayByMonth(seasonId.substring(0, 4) + "10",
                DateUtil.MONTH_UNINTERVAL, format);
        }
    }
    ...
    public static String getLastDateBySeasonId(String seasonId, String format) {
        if ("01".equalsIgnoreCase(seasonId.substring(4, 6))) {// 春
            return DateUtil.getLastDayByMonth(seasonId.substring(0, 4) + "03",
                DateUtil.MONTH_UNINTERVAL, format);
        } else if ("02".equalsIgnoreCase(seasonId.substring(4, 6))) {// 夏
            return DateUtil.getLastDayByMonth(seasonId.substring(0, 4) + "06",
                DateUtil.MONTH_UNINTERVAL, format);
        } else if ("03".equalsIgnoreCase(seasonId.substring(4, 6))) {// 秋
            return DateUtil.getLastDayByMonth(seasonId.substring(0, 4) + "09",
                DateUtil.MONTH_UNINTERVAL, format);
        } else {// 冬
            return DateUtil.getLastDayByMonth(seasonId.substring(0, 4) + "12",
                DateUtil.MONTH_UNINTERVAL, format);
        }
    }
    ...
    public static String getNextSeasonByPreSeasonId(String seasonId, String flag) {
        try {
            ...
            return getSeasonIdByDate(formatDateToStr(uDate, DateUtil.DAY_HAVINTERVAL),
                DateUtil.DAY_HAVINTERVAL);
        } catch (Exception e) {
        	logger.error(e.getMessage());
        }
     ...
     * @return
     */
    public static String[] getDateAryBeforeNDay(String date, String inFormat, String outFormat,
                                                int n) {
        if (date == null || n < 1) {
            return null;
        }
     ...
     */
    public static String[] getIntervalDays(String startDay, String endDay, String inFormat,
                                           String outFormat) {
    ...
    /**
     * 获取两个日期的年份间隔 如果有违例，请使用@exception/throws [违例类型] [违例说明：异常的注释必须说明该异常的含义及什么条件下抛出该
     * 
     * @see [类、类#方法、类#成员]
     */
    ...
    /**
     * 获取两个日期的月份间隔 如果有违例，请使用@exception/throws [违例类型] [违例说明：异常的注释必须说明该异常的含义及什么条件下抛出该
     * 
     * @see [类、类#方法、类#成员]
     */
    ...
    /**
     * 获取两个日期的天数间隔 如果有违例，请使用@exception/throws [违例类型] [违例说明：异常的注释必须说明该异常的含义及什么条件下抛出该
     * 
     * @see [类、类#方法、类#成员]
     */
    ...
    public static void main(String[] args) throws Exception {
    	...
    	System.out.println(getSysStrCurrentDate(TIME_HAVINTERVAL));
